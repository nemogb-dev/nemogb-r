...
read_canvas_grades <- function(grades) {
  ...
  assignments <- stringr::str_extract(
    names(grades),
    ".+\\s*\\(\\d+\\)"
  ) |>
    purrr::discard(is.na)
  ...
  grades$`First Name` <- (stringr::str_match(
    grades$Student,
    ".+,\\s(.+)"
  ))[, 2]

  grades$`Last Name` <- (stringr::str_match(
    grades$Student,
    "(.+),.+"
  ))[, 2]
  ...
  grades <- grades |>
    ...
    dplyr::select(c(
      "First Name", "Last Name", "SID", "Sections",
      as.vector(
        mapply(
          c,
          assignments,
