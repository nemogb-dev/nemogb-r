...
test_that("one exception - adding new category to overall grade", {
    ...
    list(
      ...
      assignments = list(
        ...
        list(
          ...
          drop_n_lowest = 1,
          aggregation = "none",
          assignments = c("Project Submission")
        )
      )
    )
  )
  ...
  gs <- tibble::tibble(SID = c(111,222,333,444,555),
                       ...
                       `Project Submission` = c(0.2, 0.5, NA, NA, NA),
                       `Project Submission - Max Points` = rep(20, 5),
                       `Project Submission - Submission Time` = rep(NA, 5),
                       `Project Submission - Lateness (H:M:S)` = rep(NA, 5)
  )
  attr(gs, "source") <- "Gradescope"
  grades <- get_grades_with_exceptions(gs, policy)
})
